...
const columns = [
  ...
  { title: "工程標題", dataIndex: "pt", key: "address", width: "400px" },
  {
    ...
    filter: [{ text: "是", value: "1" }, { text: "否", value: "0" }],
    filterMultiple: true,
      ...
      get_pmasters()
        .then(res => {
          ...
          res.list.forEach((item, i) => {
            is_bidding.add(item.is_bidding == "1" ? "是" : "否");
          });
          this.columns[4].filters = [];
          is_bidding.forEach(item => {
            if (item == "是") {
              this.columns[4].filters.push({ text: item, value: 1 });
            } else {
              this.columns[4].filters.push({ text: item, value: 0 });
            }
          });
